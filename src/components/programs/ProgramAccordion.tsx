...
/**
 * The programs accordion (`framer-KKWPz` wrapper, `framer-49uvd` items) used
 * on the home page and on /programs. Exactly one item is open, as on the
 * original, whose wrapper has one "Step" variant per program.
 *
...
function Item({ program, index, open, spec, detailsLabel, onOpen }: { program: Program; index: number; open: boolean; spec: VariantSpec; detailsLabel: string; onOpen: () => void }) {
  ...
  return (
    <div className={ITEM_CONTAINERS[index] ?? ITEM_CONTAINERS[ITEM_CONTAINERS.length - 1]} style={{ willChange: 'transform', opacity: 0, transform: 'translateY(40px)' }}>
      <div
        ref={ref}
          ...
          <div className="framer-1c0jl16" data-framer-name="Content Wrapper">
            <div className="framer-at71md" data-framer-name="Title Block">
              ...
              <div className="framer-1jm6qrw" data-framer-name="Text Block" aria-hidden={!open}>
                ...
                <Button label={detailsLabel} href={program.href} variant={spec.button} containerClass="framer-18hrayh-container" style={{ height: '100%' }} />
              </div>
            </div>
            <div className="framer-1571v0n" data-framer-name="Image Wrap">
              <div className="framer-kstmv" data-framer-name="Image" style={{ filter: 'blur(0px)', WebkitFilter: 'blur(0px)', ...RADIUS_10, transform: open ? 'none' : 'translate(-50%, -50%) scale(0)' }}>
                <div style={FILL} data-framer-background-image-wrapper="true">
                  <img decoding="async" width={program.image.width} height={program.image.height} sizes={IMAGE_SIZES} srcSet={imageSrcSet(program.image)} src={program.image.src} alt={program.image.alt} style={IMG} />
                </div>
              </div>
            </div>
          </div>
...
export function ProgramAccordion({ programs, breakpoint, containerClass, detailsLabel = 'View Details' }: { programs: Program[]; breakpoint: Breakpoint; containerClass: string; detailsLabel?: string }) {
      ...
      <div className={`framer-KKWPz framer-2vyiyj ${spec.wrapper}`} data-framer-name={spec.wrapperName} style={{ width: '100%' }}>
        <div className="framer-1m349ej" data-framer-name="Program Stack">
          {programs.map((program, i) => (
            <Item key={program.title} program={program} index={i} open={i === open} spec={spec} detailsLabel={detailsLabel} onOpen={() => setOpen(i)} />
          ))}
        </div>
